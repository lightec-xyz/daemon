...
func VerifyHandler(h http.Handler, verify IVerify) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if verify != nil {
			...
			if err != nil {
				//logger.Warn("verify jwt error: %v %v", r.URL.String(), err)
...
func verifyJwt(request *http.Request, verify IVerify) error {
	...
	if err != nil {
		//logger.Warn("get method error: %v", err)
		return err
	}
	...
	if permission != NonePermission {
		...
		if err != nil {
			logger.Error("method:%v,authorization:%v verify jwt error: %v", method, authorization, err)
...
func getMethod(data []byte) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%v,%v", err, string(data))
	}
