...
func TestProofClient_GenZkProof(t *testing.T) {
	...
	t.Log(depositProof)
	redeemProof, err := client.RedeemProof(&RedeemRequest{})
	if err != nil {
		t.Fatal(err)
	}
	t.Log(redeemProof)
	...
	t.Log(genesisProof)
	unitProof, err := client.SyncCommitUnitProve(SyncCommUnitsRequest{})
	if err != nil {
		t.Fatal(err)
	}
	t.Log(unitProof)
	recursiveProof, err := client.SyncCommDutyProve(SyncCommDutyRequest{})
	if err != nil {
		t.Fatal(err)
	}
