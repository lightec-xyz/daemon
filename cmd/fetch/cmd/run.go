...
func NewFetch(url, datadir string, genesisSlot uint64) (*Fetch, error) {
	...
	tables := []store.Table{common.GenesisTable, common.IndexTable,
		common.FinalityTable, common.UpdateTable}
	...
	fetch, err := node.NewFetch(client, dbStore, fileStorage, genesisSlot, nil, nil)
	if err != nil {
